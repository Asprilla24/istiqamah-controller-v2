package id.co.aladine.istiqamahcontroller.animation;
...
public class ShadowTransformer implements ViewPager.OnPageChangeListener, ViewPager.PageTransformer {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (currentCard != null) {
            if (scalingEnabled) {
                currentCard.setScaleX((float) (1 + scaling * (1 - realOffset)));
                currentCard.setScaleY((float) (1 + scaling * (1 - realOffset)));
            }
...
//                    * (CardAdapter.MAX_ELEVATION_FACTOR - 1) * (1 - realOffset)));
        ...
        if (nextCard != null) {
            if (scalingEnabled) {
                nextCard.setScaleX((float) (1 + scaling * (realOffset)));
                nextCard.setScaleY((float) (1 + scaling * (realOffset)));
            }
